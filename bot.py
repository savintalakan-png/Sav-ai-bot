...
async def premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'chat':
        ...
        await query.edit_message_text(
            f"💬 **Режим чата с ИИ**\n\nОсталось сообщений сегодня: {remaining if remaining != float('inf') else '∞'}\n\nНапишите мне что-нибудь!",
            parse_mode='Markdown'
        )
    elif data == 'image':
    ...
    if not db.can_send_message(user_id):
        await update.message.reply_text("❌ Вы исчерпали лимит сообщений на сегодня.\nПриходите завтра или активируйте Premium командой /premium")
        return
    if state == 'chat':
        ...
        if error:
            ...
            await update.message.reply_text(f"{response}\n\n🟢 {provider} · Осталось: {remaining_text}")
    elif state == 'image':
